...
```sh
# Extract the MTP head from the official FP8 checkpoint.
...
`--draft-block-size` counts draft tokens plus one target token: `2` proposes
...
- `--draft-model`: GLM-5.3-Flash native MTP checkpoint path or HF id
- `--draft-kind`: `mtp`
