...
def load_mlx_models(path):
    model = NanoLlavaModel.from_pretrained(path)
    return model
...
class TestVisionTower(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mx_nanoLlava = load_mlx_models(MODEL_PATH)
        cls.hf_nanoLlava = load_hf_models(MODEL_PATH)
    ...
    def test_image_features(self):
        ...
        with torch.no_grad():
            ...
            _, _, hidden_states = self.mx_nanoLlava.vision_tower(
                mx_pixel_values,
                output_hidden_states=True,
            )

            mx_elected_image_feature = hidden_states[vision_feature_layer]
            mx_image_features = self.mx_nanoLlava.mm_projector(
                mx_elected_image_feature
            )

            hf_image_outputs = self.hf_nanoLlava.vision_tower(
                hf_pixel_values, output_hidden_states=True
            )
            hf_elected_image_feature = hf_image_outputs.hidden_states[
                vision_feature_layer
            ]
            hf_image_features = self.hf_nanoLlava.mm_projector(
...
class TestLlava(unittest.TestCase):
    ...
    def test_merge_input_ids_with_image_features(self):
        ...
        with torch.no_grad():
            ...
            mx_inputs_embeds = self.mx_nanoLlava.language_model.model.embed_tokens(
                mx_input_ids
            )
            mx_final_embedding = self.mx_nanoLlava._prepare_inputs_for_multimodal(
            ...
            hf_inputs_embeds = self.hf_nanoLlava.get_input_embeddings()(input_ids)
            hf_final_embedding, _, _, _ = (
                self.hf_nanoLlava._prepare_inputs_for_multimodal(
                    hf_image_features,
                    hf_inputs_embeds,
                    input_ids,
    ...
    def test_generated_tokens(self):
        ...
        with torch.no_grad():
            ...
            hf_outputs = self.hf_nanoLlava(**hf_inputs)
            ...
            mx_logits, _ = self.mx_nanoLlava(input_ids, pixel_values)
