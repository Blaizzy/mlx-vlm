...
def process_image(img, resize_shape, image_processor):
    ...
    if resize_shape is not None and not isinstance(image_processor, BaseImageProcessor):
        # Resize to exact dimensions instead of fitting within
        img = img.resize(resize_shape, Image.Resampling.BICUBIC)
    return img
...
def process_inputs(
    ...
    **kwargs,
    ...
    # Add images only if it's not None and supported by the process method
    if images is not None and "images" in inspect.signature(process_method).parameters:
        args["images"] = images
    # Make sure images is not in args if it's None
    if images is None and "images" in args:
        del args["images"]
...
def prepare_inputs(
    ...
    **kwargs,
    ...
    if hasattr(processor, "image_processor") and isinstance(
        ...
        # For processors that don't support images directly, process text separately
        inputs = process_inputs_with_fallback(
            processor,
            prompts=prompts,
            images=None,  # Don't pass images to tokenizer
            audio=audio,
            add_special_tokens=add_special_tokens,
            **kwargs,
        )

        # Add processed images back to inputs
        if images is not None:
            inputs["pixel_values"] = mx.array(np.stack(images))
