...
def test_generate_step_block_size_one_decodes_without_the_drafter():
    """A block of one leaves the drafter nothing to propose.

    The round loops read a block size of one as an exhausted budget and stop, so
    asking for it used to end the generation after a single token.
    """
    model = MagicMock()
    model.language_model.supports_logits_to_keep = False
    model.language_model.return_value = MagicMock(
    ...
    drafter = MagicMock()
    drafter.draft_block.side_effect = AssertionError("drafter must not be used")
    ...
    assert len(tokens) == 4
    drafter.draft_block.assert_not_called()
