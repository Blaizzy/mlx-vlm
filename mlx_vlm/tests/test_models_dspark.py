...
def test_sampled_dspark_generation_matches_baseline(
    target_factory, draft_config_factory, temperature
):
    mx.random.seed(37)
    target = target_factory()
    drafter = DSparkDraftModel(draft_config_factory())
    ...
    baseline = _generated_tokens(
        target, prompt, max_tokens=24, temperature=temperature, seed=41
    )
    speculative = _generated_tokens(
        target, prompt, drafter, max_tokens=24, temperature=temperature, seed=41
    )
