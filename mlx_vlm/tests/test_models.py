...
class TestQwen3_5TextOnly(unittest.TestCase):
    """Text-only qwen3_5 checkpoints keep the decoder fields at the top level."""
    ...
    def test_model_type_routes_to_qwen3_5(self):
        from mlx_vlm.utils import MODEL_REMAPPING

        self.assertEqual(MODEL_REMAPPING["qwen3_5_text"], "qwen3_5")

    def test_flat_config_builds_text_config_without_vision(self):
        from mlx_vlm.models.qwen3_5 import ModelConfig

        config = ModelConfig.from_dict(copy.deepcopy(self.FLAT_CONFIG))
    ...
    def test_text_only_model_skips_the_vision_tower(self):
        from mlx_vlm.models.qwen3_5 import Model, ModelConfig

        config = ModelConfig.from_dict(copy.deepcopy(self.FLAT_CONFIG))
        model = Model(config)
        self.assertIsNone(model.vision_tower)
        logits = model.language_model(mx.array([[0, 1, 2]])).logits
        self.assertEqual(logits.shape, (1, 3, config.text_config.vocab_size))
