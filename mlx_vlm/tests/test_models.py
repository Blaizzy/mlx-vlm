...
def test_glm5_next_speculative_verify_matches_sequential_decode_arithmetic():
    ...
    prompt = mx.array([[1, 2, 3]], dtype=mx.int32)
    # Exercise the longest fixed MTP block used by the GLM verifier.  In
    # particular, MoE routing must retain decode-shaped Bx1 arithmetic rather
    # than flattening the six time steps into the batch dimension.
    verify_tokens = mx.array([[4, 5, 6, 7, 8, 9]], dtype=mx.int32)
...
def test_glm5_next_indexer_tail_slot_is_independent_of_other_row_length():
    mx.random.seed(16)
    model = LanguageModel(_text_config())
    model.eval()
    indexer = model.layers[1].self_attn.indexer

    def final_topk(tokens, attention_mask, left_padding):
        captured = []
        original = Glm5NextIndexer.__call__

        def recording(current, *args, **kwargs):
            topk = original(current, *args, **kwargs)
            if current is indexer and args[0].shape[1] == 1:
                captured.append(topk)
            return topk

        cache = _make_cache(model, left_padding=left_padding)
        with patch.object(Glm5NextIndexer, "__call__", recording):
            model(
                tokens[:, :-1],
                cache=cache,
        ...
        mx.eval(captured[-1])
        return captured[-1][0, 0]

    short_batch = final_topk(
        mx.array([[1, 2, 3], [4, 5, 6]], dtype=mx.int32),
        mx.ones((2, 3), dtype=mx.bool_),
        [0, 0],
    )
    longer_peer = final_topk(
        mx.array([[0, 1, 2, 3], [4, 5, 6, 7]], dtype=mx.int32),
        mx.array([[0, 1, 1, 1], [1, 1, 1, 1]], dtype=mx.bool_),
        [1, 0],
    )

    short_valid = short_batch >= 0
    peer_valid = longer_peer >= 0
    mx.eval(short_valid, peer_valid)
    assert short_valid.tolist() == [True, True, False, False, True]
    assert peer_valid.tolist() == short_valid.tolist()
