...
class TestModels(unittest.TestCase):
    ...
    def test_glm5_next_fused_in_proj_mxfp8_fallback(self):
        import mlx.nn as nn
        ...
        cache[0] = None
        cache[1] = None
        output = kda(mx.random.normal((1, 3, cfg.hidden_size)), None, cache)
        self.assertFalse(kda.fuse_in)
        self.assertEqual(output.shape, (1, 3, cfg.hidden_size))
    ...
    def test_glm5_next_mxfp8_verify_matches_singleton_decode(self):
        import mlx.nn as nn
    ...
    def test_glm5_next_mtp_split_dequantizes_fine_grained_fp8(self):
        ...
        splitter = Glm5NextMTPSplitter()
        sanitized = splitter.run_sanitize(
            {weight_key: weight, f"{weight_key}_scale_inv": scale_inv},
            {"num_hidden_layers": 1},
        )
        actual = sanitized["mtp.self_attn.q_proj.weight"]
        expected = mx.from_fp8(weight, dtype=mx.bfloat16) * scale_inv[0, 0]
        mx.eval(actual, expected)
        self.assertTrue(mx.array_equal(actual, expected).item())
        self.assertFalse(any(key.endswith("weight_scale_inv") for key in sanitized))
