...
class TestGetInputEmbeddings(unittest.TestCase):
    ...
    def test_qwen3_omni_moe_text_only_clears_mrope_state(self):
        from mlx_vlm.models import qwen3_omni_moe
        ...
        language_model = model.thinker.language_model
        language_model._position_ids = mx.array([[[0, 1]]], dtype=mx.int32)
        language_model._rope_deltas = mx.array([[4]], dtype=mx.int32)

        model.thinker.get_input_embeddings(mx.array([[1, 2]], dtype=mx.int32))

        self.assertIsNone(language_model._position_ids)
        self.assertIsNone(language_model._rope_deltas)

    def test_qwen3_omni_moe_prefill_recomputes_stale_position_ids(self):
        from mlx_vlm.models import qwen3_omni_moe
        ...
        language_model = qwen3_omni_moe.LanguageModel(text_config, thinker_config)
        ...
        language_model.model = _CapturingInnerModel()
        language_model.lm_head = lambda x: x
        language_model._position_ids = mx.array([[[0, 1, 2]]], dtype=mx.int32)
        language_model._rope_deltas = mx.array([[3]], dtype=mx.int32)

        class _Cache:
            _idx = 0
            offset = mx.array(0)

        language_model(mx.array([[1, 2], [3, 4]], dtype=mx.int32), cache=[_Cache()])

        self.assertEqual(captured["position_ids"].shape, (3, 2, 2))
        self.assertEqual(language_model._rope_deltas.tolist(), [[0], [0]])
        ...
        with patch("mlx_vlm.models.qwen3_omni_moe.language.mx.eval") as eval_mock:
            language_model(
        ...
        self.assertEqual(captured["position_ids"][0, 0, 0].item(), 15)
