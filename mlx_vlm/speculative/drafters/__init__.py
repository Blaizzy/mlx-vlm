...
# here keeps whatever ``kind`` the caller passed (typically ``"dflash"``).
...
def resolve_drafter_kind(model_path, kind: str) -> str:
    """Reconcile the caller's ``kind`` with the drafter's actual model type.

    If the drafter's ``model_type`` maps to a different round-loop kind than
    the caller passed, we override (and warn). This avoids the trap where a
...
def load_drafter(
    path_or_repo: str, kind: str = "dflash", **kwargs
) -> Tuple[object, str]:
    """Load a speculative drafter and return ``(model, resolved_kind)``.

    ``resolved_kind`` may differ from ``kind`` when the drafter's HF
    ``model_type`` implies a specific round-loop (see
    :func:`resolve_drafter_kind`). Callers should use ``resolved_kind`` for
    downstream dispatch instead of trusting their original ``kind`` arg.
    """
