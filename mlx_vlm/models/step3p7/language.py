...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        if self.limit is not None:
            return self.down_proj(_clamped_swiglu(self.up_proj(x), self.gate_proj(x), self.limit))
...
class Attention(nn.Module):
    def __init__(self, config: TextConfig, layer_idx: int):
        ...
        theta = config.rope_theta[layer_idx] if isinstance(config.rope_theta, list) else config.rope_theta
        partial = 1.0
        ...
        rope_scaling = None if config.yarn_only_types and layer_type not in config.yarn_only_types else config.rope_scaling
...
class DecoderLayer(nn.Module):
    def __init__(self, config: TextConfig, layer_idx: int):
        ...
        self.post_attention_layernorm = StepRMSNorm(config.hidden_size, config.rms_norm_eps)
...
class StepTextModel(nn.Module):
    def __init__(self, config: TextConfig):
        ...
        self._full_idx = next((i for i, layer in enumerate(self.layers) if not layer.is_sliding), None)
    ...
    def __call__(self, input_ids, inputs_embeds=None, cache=None):
        ...
        full_mask = create_attention_mask(h, cache[self._full_idx]) if self._full_idx is not None else None
        swa_mask = (
            create_attention_mask(h, cache[self._swa_idx], window_size=self.args.sliding_window)
        ...
        for key, value in weights.items():
            ...
            if "model.layers." in key:
                ...
                if "layers" in parts:
                    ...
                    if layer_pos + 1 < len(parts) and parts[layer_pos + 1].isdigit() and int(parts[layer_pos + 1]) >= self.args.num_hidden_layers:
                        continue
