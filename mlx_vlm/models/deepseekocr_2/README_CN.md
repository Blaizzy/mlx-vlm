...
DeepSeek-OCR-2 是一个强大的 OCR 模型，基于 SAM + Qwen2 编码器架构，针对文档理解、文本提取和视觉定位任务进行了优化。
...
prompt = f"Locate <|ref|>{text_toFind}<|/ref|> in the image."
formatted_prompt = apply_chat_template(processor, model.config, prompt, num_images=1)
...
DeepSeek-OCR-2 使用动态分辨率来高效处理不同大小的图像：
