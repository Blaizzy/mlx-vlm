...
def _target_verify_fused_mxfp4_qmv_source(n_sizes) -> str:
    selections = []
    offset = int(n_sizes[0])
    for index, n_size in enumerate(n_sizes[1:], start=1):
        selections.append(f"""    if (global_out >= {offset}) {{
...
@lru_cache(maxsize=None)
def _target_verify_mxfp4_qmv_kernel(dtype, verify_t, k_size, n_size):
    dtype_name = {mx.bfloat16: "bf16", mx.float16: "fp16"}.get(dtype, "unk")
    return mx.fast.metal_kernel(
        name=(
            "qwen3_5_target_verify_mxfp4_qmv_"
...
@lru_cache(maxsize=None)
def _target_verify_mxfp4_qargmax_kernel(dtype, verify_t, k_size, n_size):
    dtype_name = {mx.bfloat16: "bf16", mx.float16: "fp16"}.get(dtype, "unk")
    return mx.fast.metal_kernel(
        name=(
            "qwen3_5_target_verify_mxfp4_qargmax_"
            f"t{verify_t}_k{k_size}_n{n_size}_{dtype_name}"
        ),
        input_names=["x", "w", "scales"],
...
@lru_cache(maxsize=None)
def _target_verify_fused_mxfp4_qmv_kernel(dtype, verify_t, k_size, n_sizes):
    dtype_name = {mx.bfloat16: "bf16", mx.float16: "fp16"}.get(dtype, "unk")
    shape_name = "_".join(str(n) for n in n_sizes)
    input_names = ["x"]
    for index in range(len(n_sizes)):
    ...
    return mx.fast.metal_kernel(
        name=(
            "qwen3_5_target_verify_fused_mxfp4_qmv_"
            f"t{verify_t}_k{k_size}_n{shape_name}_{dtype_name}"
        ),
        input_names=input_names,
        output_names=["y"],
        header=_TARGET_VERIFY_MXFP4_HEADER,
        source=_target_verify_fused_mxfp4_qmv_source(n_sizes),
    )
...
def _target_verify_mxfp4_linear(linear, x: mx.array) -> Optional[mx.array]:
    ...
    N = linear.weight.shape[0]
    x = mx.contiguous(x)
    kernel = _target_verify_mxfp4_qmv_kernel(x.dtype, T, K, N)
    out = kernel(
...
def _target_verify_mxfp4_linears(linears, x: mx.array):
    if (
        not 2 <= len(linears) <= 4
        or x.ndim != 3
        or not 1 < x.shape[1] <= 4
        or not all(
            "bias" not in linear and _can_target_verify_mxfp4(linear, x)
    ...
    total_n = sum(n_sizes)
    x = mx.contiguous(x)
    kernel = _target_verify_fused_mxfp4_qmv_kernel(x.dtype, T, K, n_sizes)
    inputs = [x]
    for linear in linears:
