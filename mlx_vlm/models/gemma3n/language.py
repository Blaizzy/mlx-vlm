...
class MLP(nn.Module):
    def __init__(self, config: TextConfig, layer_idx: int = 0):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size[0], bias=False)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size[0], bias=False)
        self.down_proj = nn.Linear(self.intermediate_size[0], self.hidden_size, bias=False)
