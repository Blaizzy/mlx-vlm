...
class VisionEmbeddings(nn.Module):
    ...
    def _get_pos_embed(self, pos_embed, H, W):
        ...
        orig_H = orig_W = self.image_size // self.patch_size

        # Reshape to spatial format: (1, embed_dim, orig_H, orig_W)
        # The input pos_embed excludes the class token embedding
        pos_embed_spatial = (
            pos_embed.astype(mx.float32)
            .reshape(1, orig_H, orig_W, embed_dim)
            .transpose(0, 3, 1, 2)
        )
        ...
        # Apply Upsample layer for bicubic interpolation
        # Since H, W can vary per call, instantiate the layer dynamically
        upsampler = nn.Upsample(
            scale_factor=(scale_H, scale_W), mode="cubic", align_corners=False
        )
        upsampled_embed = upsampler(pos_embed_spatial)
        # Output shape: (1, embed_dim, H, W)

        # Reshape back to sequence format: (1, H * W, embed_dim)
        pos_embed = upsampled_embed.reshape(1, -1, embed_dim)  # .transpose(0, 2, 1)

        return pos_embed.astype(target_dtype)
