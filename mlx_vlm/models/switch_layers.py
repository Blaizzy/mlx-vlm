...
class FusedSwitchGLU(SwitchGLU):
    """SwitchGLU with gate and up projections stored in one linear.

    Checkpoint tensors are stored gate-first to match ordinary fused MLPs.  A
    single gathered QMM avoids reading routing metadata and dispatching the
    same input twice while preserving the exact quantized projection results.
    """
    ...
    def __call__(
        ...
        if not self.training and x.ndim == 3 and 1 < x.shape[1] <= DECODE_BLOCK_SIZE:
            ...
            gate_up = exact_quantized_switch_linear(self.gate_up_proj, x, indices)
        ...
        routed = self.down_proj(self.activation(up, gate), idx, sorted_indices=do_sort)
        if do_sort:
            routed = _scatter_unsort(routed, inv_order, indices.shape)
        return self._combine(routed.squeeze(-2), weights, shared, residual)
