...
class Molmo2Transformer(nn.Module):
    ...
    def __call__(
        ...
        if inputs_embeds is None:
            ...
            hidden_states = self.wte(safe_ids)
        else:
        ...
        pre_ln = hidden_states
        hidden_states = self.ln_f(hidden_states)

        if return_pre_ln:
            return hidden_states, pre_ln
        return hidden_states


class ExtendedLmHead(nn.Module):
    """LM head with split output/new embeddings (151936 + 128)."""

    def __init__(self, vocab_size: int, additional_vocab_size: int, hidden_size: int):
        super().__init__()
        self.output_embeddings = mx.zeros((vocab_size, hidden_size))
        self.new_output_embeddings = mx.zeros((additional_vocab_size, hidden_size))
        self._fused = None

    def fuse(self):
        """Pre-concat the two weight matrices."""
        self._fused = mx.concatenate(
    ...
    def __call__(self, hidden_states: mx.array) -> mx.array:
        if self._fused is None:
            self.fuse()
        return hidden_states @ self._fused.T
