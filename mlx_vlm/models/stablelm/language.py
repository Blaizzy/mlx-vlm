...
class StableLM(nn.Module):
    ...
    def __call__(self, x, cache, inputs_embeds=None):
        x = self.embed_tokens(x) if inputs_embeds is None else inputs_embeds
        if cache is None:
...
class LanguageModel(nn.Module):
    ...
    def __call__(
        ...
        y = self.model(x, cache, inputs_embeds=inputs_embeds)
