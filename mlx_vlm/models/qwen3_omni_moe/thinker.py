...
class Thinker(nn.Module):
    def __init__(self, config: ThinkerConfig):
        ...
        """need to sanitize, visual -> vision_tower"""
        self.vision_tower = VisionModel(config.vision_config)

        self.audio_tower = AudioModel(config.audio_config)  # todo: to be implemented

        """need to sanitize, model -> language_model.model"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # todo: 需要参考gemma3n适配音频输入，更新inputs_embeds
        ...
        for key, value in weights.items():
            if "thinker" in key:
                if "thinker.model" in key:
                    ...
                    # OMNI EXPERT GATE/UP/DOWN are impl by Linear, which have 2 dims, we dont need to swap dims here.
                elif "thinker.visual" in key:
                    key = key.replace("thinker.visual", "thinker.vision_tower")
                    if "merger" in key:
                        key = key.replace("ln_q", "norm").replace("mlp.0", "linear_fc1").replace("mlp.2", "linear_fc2")
                    if "merger_list" in key:
                        key = key.replace("merger_list", "deepstack_merger_list")

                
                # thinker.audio_tower don't need to be modified.

                if "thinker.lm_head" in key:
                    key = key.replace("thinker.lm_head", "thinker.language_model.lm_head")
