...
class Griffin(nn.Module):
    ...
    def __call__(self, tokens, cache=None, inputs_embeds=None):
...
class LanguageModel(nn.Module):
    ...
    def __call__(
        self, tokens: mx.array, cache=None, inputs_embeds=None, mask=None, **kwargs
    ) -> mx.array:
        logits = self.model(tokens, cache=cache, inputs_embeds=inputs_embeds)
        if "lm_head" in self:
