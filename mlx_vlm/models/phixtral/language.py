...
class MOE(nn.Module):
    def __init__(self, args: ModelConfig, dim: int, hidden_dim: int):
        super().__init__()
        self.dim = dim
...
class TransformerDecoder(nn.Module):
    ...
    def __call__(self, x, mask, cache, inputs_embeds=None):
        x = self.embd(x) if inputs_embeds is None else inputs_embeds
        if cache is None:
...
class LanguageModel(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if mask is None and inputs_embeds is not None:
            mask = create_attention_mask(inputs_embeds, cache)
        elif mask is None:
            mask = create_attention_mask(x, cache)

        y = self.transformer(x, mask, cache, inputs_embeds=inputs_embeds)
