...
# Special tokens
_COMPATIBLE_IMAGE_SPECIAL_TOKEN_PATTERN = (
    r"<\|image_\d+\|>"  # For backward compatibility
)
_COMPATIBLE_AUDIO_SPECIAL_TOKEN_PATTERN = (
    r"<\|audio_\d+\|>"  # For backward compatibility
)
...
class Phi4MMAudioFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __init__(
        self, audio_compression_rate, audio_downsample_rate, audio_feat_stride, **kwargs
    ):
        feature_size = 80
        sampling_rate = 16000
        padding_value = 0.0
    ...
    def __call__(
        ...
        returned_audio_attention_mask = (
            torch.arange(0, audio_frames.max()).unsqueeze(0) < audio_frames.unsqueeze(1)
            if len(audios) > 1
            else None
        )
...
class Phi4MMProcessor(ProcessorMixin):
    ...
    image_processor_class = (
        "AutoImageProcessor"  # Phi4MMImageProcessor will be registered later
    )
    audio_processor_class = (
        "AutoFeatureExtractor"  # Phi4MMAudioFeatureExtractor will be registered later
    )
    ...
    def __call__(
        ...
        """
        image_inputs = (
            self.image_processor(images, return_tensors=return_tensors)
            if images is not None
            else {}
        )
        audio_inputs = (
            self.audio_processor(audios, return_tensors=return_tensors)
    ...
    def _convert_images_audios_text_to_inputs(
        ...
        processed_text = [
            re.sub(_COMPATIBLE_IMAGE_SPECIAL_TOKEN_PATTERN, _IMAGE_SPECIAL_TOKEN, t)
            for t in text
        ]
        processed_text = [
            re.sub(_COMPATIBLE_AUDIO_SPECIAL_TOKEN_PATTERN, _AUDIO_SPECIAL_TOKEN, t)
        ...
        max_len = lengths.max()
        input_ids = input_ids.new_full(
            (len(new_input_ids_list), max_len), self.tokenizer.pad_token_id
        )
...
AutoFeatureExtractor.register(
    "Phi4MMAudioFeatureExtractor", Phi4MMAudioFeatureExtractor
)
