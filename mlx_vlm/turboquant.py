...
class TurboQuantKVCache(_BaseCache):
    ...
    def prefill_attention(
        ...
        """Flash-style prefill: tile along KV sequence with online softmax.

        Like MLX's flash attention, this holds Q constant and streams through
        KV in chunks, never materializing the full (B, H, L, T) score matrix.
        Each chunk produces partial scores → partial exp-weights → partial
        value sums, combined via the online softmax rescaling algorithm.

        Memory is bounded by O(L × chunk_T) per chunk instead of O(L × T).
        """
        if keys_state is None or values_state is None:
            keys_state, values_state = self.state
        keys_state = self._unwrap(keys_state)
        ...
        # Choose chunk_T to bound peak score memory at ~512 MB.
        # Score matrix per chunk: B * n_kv_heads * n_repeats * L * chunk_T * 4 bytes.
        score_row_bytes = B * n_kv_heads * n_repeats * L * 4
        target_bytes = 512 * 1024 * 1024
        tok_tile_size = 1024
        if score_row_bytes > 0:
            chunk_T = target_bytes // score_row_bytes
            chunk_T = max(tok_tile_size, (chunk_T // tok_tile_size) * tok_tile_size)
        else:
            chunk_T = T
        chunk_T = min(chunk_T, T)

        # Prepare queries once — held constant across KV chunks (flash-style).
        ...
        # Online softmax accumulators (Welford-style, same as MLX flash attn).
        acc = mx.zeros(
            (B, n_kv_heads, n_repeats, L, D), dtype=mx.float32
        )
        denom = mx.zeros(
            (B, n_kv_heads, n_repeats, L), dtype=mx.float32
        )
        running_max = mx.full(
            (B, n_kv_heads, n_repeats, L), -1e38, dtype=mx.float32
        )

        # Stream through KV in chunks — mirror of flash attention's KB loop.
        for k_start in range(0, T, chunk_T):
            k_end = min(T, k_start + chunk_T)
            ct = k_end - k_start

            # Slice quantized KV for this chunk.
            key_chunk = _slice_state_range(keys_state, k_start, k_end)
            val_chunk = _slice_state_range(values_state, k_start, k_end)

            # Score: (B*H*R, L, ct) via Metal kernel.
            chunk_scores = mq_score(
                inputs=[
                    q_rot,
                    q_proj,
                    key_chunk.norms,
                    key_chunk.mse_indices,
                    key_chunk.residual_norms,
                    key_chunk.qjl_signs,
                ...
                template=[
                    ("Dim", D),
                    ("KMsePackedWidth", key_chunk.mse_indices.shape[-1]),
                    ("KSignPackedWidth", key_chunk.qjl_signs.shape[-1]),
                ],
                grid=(32, n_repeats, B * n_kv_heads * ct),
                threadgroup=(32, 1, 1),
                output_shapes=[(B * n_kv_heads * n_repeats, L, ct)],
                output_dtypes=[mx.float32],
            )[0]
            chunk_scores = chunk_scores.reshape(
                B, n_kv_heads, n_repeats, L, ct
            )

            # Apply mask for this chunk range.
            if mask is not None:
                if isinstance(mask, mx.array):
                    mask_chunk = mask[..., k_start:k_end]
                    if mask_chunk.ndim == chunk_scores.ndim - 1:
                        mask_chunk = mx.expand_dims(mask_chunk, axis=2)
                    if mask_chunk.dtype == mx.bool_:
                        chunk_scores = mx.where(
                            mask_chunk,
                            chunk_scores,
                            mx.finfo(chunk_scores.dtype).min,
                        )
                    else:
                        chunk_scores = chunk_scores + mask_chunk
                elif isinstance(mask, str):
                    return None

            # Chunk softmax stats: max and unnormalized exp-weights.
            chunk_max = mx.max(chunk_scores, axis=-1)  # (B, H, R, L)
            chunk_exp = mx.exp(chunk_scores - chunk_max[..., None])
            chunk_denom = mx.sum(chunk_exp, axis=-1)  # (B, H, R, L)

            # Value weighted sum for this chunk (precomputed weights path).
            chunk_weights = chunk_exp.reshape(
                B * n_kv_heads, n_repeats * L, ct
            )
            num_ct_tiles = (ct + tok_tile_size - 1) // tok_tile_size
            out_shape = (
                B * n_kv_heads * num_ct_tiles,
                n_repeats * L,
                D,
            )
            ...
            out_tiled = val_kernel(
                ...
                template=[
                    ...
                    ("PackedWidth", val_chunk.indices.shape[-1]),
                ],
                grid=(D, 1, B * n_kv_heads * num_ct_tiles),
                threadgroup=(D, 1, 1),
                output_shapes=[out_shape],
                output_dtypes=[mx.float32],
            )[0]

            # Reduce value tiles within this chunk.
            out_tiled = out_tiled.reshape(
                B * n_kv_heads, num_ct_tiles, n_repeats * L, D
            )
            if num_ct_tiles > 1:
                chunk_val = mx.sum(out_tiled, axis=1)
            else:
                chunk_val = out_tiled.squeeze(1)
            chunk_val = chunk_val.reshape(
                B, n_kv_heads, n_repeats, L, D
            )

            # Online softmax merge (same algorithm as MLX flash attention):
            #   new_max = max(running_max, chunk_max)
            #   prev_scale = exp(running_max - new_max)
            #   chunk_scale = exp(chunk_max - new_max)
            #   acc = acc * prev_scale + chunk_val * chunk_scale
            #   denom = denom * prev_scale + chunk_denom * chunk_scale
            new_max = mx.maximum(running_max, chunk_max)
            prev_scale = mx.exp(running_max - new_max)
            chunk_scale = mx.exp(chunk_max - new_max)

            acc = (
                acc * prev_scale[..., None]
                + chunk_val * chunk_scale[..., None]
            )
            denom = denom * prev_scale + chunk_denom * chunk_scale
            running_max = new_max

            # Eval per chunk to bound the compute graph and memory.
            mx.eval(acc, denom, running_max)

        # Final normalization and rotation.
        output = acc / mx.maximum(denom[..., None], _EPS)
        output = mx.matmul(output, self.value_codec.rotation)
